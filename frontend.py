...
class ApiHandler(webapp.RequestHandler):
    ...
    '''Handle a GET request to the API.'''
        self.response.out.write('Got request')
    ...
    def handle_exception(self, exception, debug_mode):
        ...
        '''
        if isinstance(exception, common.ServerError): # Server error: notify user.
            self.error(500)
        ...
        else: # Send all else to Google.
            super(ApiHandler, self).handle_exception(exception, True)


class ServeHandler(webapp.RequestHandler):
    """
    Take a URL with encoded output information from the ApiHandler, decode it,
