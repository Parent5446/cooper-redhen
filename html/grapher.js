        ...
        $('#compare_button').click(function() {
            ...
            var got_results = function(response) { //Once the server has responded
                ...
                $('#graph').mousemove( function(e) { //Add functions to the graph
                    ...
                    var x = Math.floor(e.pageX - $('#graph_box').position().left);
                    var y = Math.floor(e.pageY - $('#graph_box').position().top);
                    if(dragging)
                    {
                        $('#graph').css('left', x-startx);
                        $('#graph').css('top', y-starty);
                        //startx = x; starty = y;
                    }
                ...
                $('#graph').mousedown( function(e) {
                    ...
                    var y = Math.floor(e.pageY - $('#graph_box').position().top);
                    startx = x; starty = y;
                    for(var i in spectra) if(Math.abs(spectra[i].data[x]-($('#graph_box').height()-y))<15) { selected=i; break; }
                });
                $('#graph').mouseup( function(e) { dragging = false; });
